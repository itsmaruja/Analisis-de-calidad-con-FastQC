...
###Se ouede ver que el fastq2 sigue teniendo el primer ya que las secuencias arrancan todas con AATTCC
...
report <- trimLRPatterns(Lpattern = "AATTCCGGTT", subject = sampler, max.Rmismatch = 0, ranges = TRUE)

View(as.data.frame(report))

#arreglado
fq2 <- trimLRPatterns(Lpattern = "AATTCCGGTT", subject = sampler, max.Rmismatch = 0, ranges = TRUE)
...
rqcResult <- rqcQA(sampler7, pair=c(1,1), workers=1)

# Obtener porcentajes de GC por ciclo
gcContent <- perCycleBase(gcContent(rqcResult))

# Graficar contenido de GC
rqcCycleGCPlot(gcContent)

# Filtrar ciclos con alto contenido de GC (>50%)
highGC <- gcContent > 50

# Graficar los ciclos con contenido de GC aceptable
acceptableGC <- gcContent <= 50
rqcCycleGCPlot(acceptableGC)

length(sampler7fixed)
length(removed7)
